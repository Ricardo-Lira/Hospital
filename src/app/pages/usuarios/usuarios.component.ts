...
export class UsuariosComponent implements OnInit {
  ...
  borrarUsuario(usuario:Usuario){
    ...
    if(usuario._id === this._usuarioService.usuario._id){
      swal('No puede Borrar el Usuario', 'No puede Borrarse asi mismo', 'error' );
    ...
    swal({
      title: '¿Estas Seguro?',
      text: 'Estas a pundo de borrar a ' + usuario.nombre,
      icon: 'warning',
      buttons: true,
      dangerMode: true, 
    })
    .then(borrar =>{
      ...
      if(borrar){
        this._usuarioService.borrarUsuario(usuario._id).subscribe(borrado => {
         /*  window.location.assign('http://localhost:4200/#/usuarios') */
          this.cargarUsuarios();
